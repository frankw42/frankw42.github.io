...
;---
;--- Learning exercise, Frank Wroblewski 
;--- Add min Owl animation - 6/10/2018
;--- Mouse click steps throught eye blink
;---

  (js/alert "owl start 2 ") 
...
(defonce owl-state (atom owl-starting-state))
...
(defn blinkToggle [e]
        (swap! owl-state update-in [:eyes] cycle3)    
        (.preventDefault e)           
)
...
(defn main-template [{:keys [score cur-time jump-count
 ...
 (defn owl-template [{:keys [eyes]}]
  (sab/html [:div.owl { :onMouseDown (fn [e]
                             ;;(js/alert "owl fire???") 
                             (blinkToggle e)
                             )}   
     ...
     [:img {:key "img1"  :src (str "imgs/owl/background.png")  :style {:display "inline-block"  :position "absolute" } } ]       
           ...
           (when (= 0 eyes) (do     
               [:img {:key "img1"  :src (str "https://frankw42.github.io/owl/public/imgs/owl/background.png")  :style {:display "inline-block"  :position "absolute" } } ]
             ))
             (when (= 1 eyes) (do
                 [:img {:key "img2"  :src (str "https://frankw42.github.io/owl/public/imgs/owl/blinkHalf-c.png") :style {:display "inline-block"          :position "absolute" :left 61 :top 38 } } ]
             ))
             (when (= 2 eyes) (do
                 [:img {:key "img3"  :src (str "https://frankw42.github.io/owl/public/imgs/owl/blinkFull-c.png") :style {:display "inline-block"  :position "absolute" :left 61 :top 38 } } ] 
             ))       
    ])
 )                    

(let [node (.getElementById js/document "board-area")
      owl (.getElementById js/document "owl-area")
]
  ...
  (defn renderer-owl [full-state]
    (.render js/ReactDOM (owl-template full-state) owl)
  )   
)
...
(add-watch owl-state :renderer (fn [_ _ _ n]
                                  (renderer-owl (world n))))                                 
                                  

(reset! flap-state @flap-state)

(reset! owl-state @owl-state)
